...
class tracker():
    ...
    def __init__(self, Mywindow_width, Mywindow_height, Mymargin, My_ym = 1, My_xm = 1, Mysmooth_factor = 15, hit_fraction = 0.7):
    ...
    def find_window_centroids(self, warped, best_horizon):
            m = best_horizon[0]
            b = best_horizon[1]
            window_width = self.window_width
            window_height = self.window_height
            hit_number = window_width * window_height * self.hit_fraction * 255
            margin = self.margin
            ...
            #sum tenth bottom of image to get slice, could use a different ratio
            l_sum = np.sum(warped[int(9*warped.shape[0]/10):,:int(warped.shape[1]/2)], axis=0)
            l_center = np.argmax(np.convolve(window,l_sum))-window_width/2
            r_sum = np.sum(warped[int(9*warped.shape[0]/10):,int(warped.shape[1]/2):], axis=0)
            ...
            for level in range(1,(int)(4*warped.shape[0]/window_height)):
                ...
                image_layer = np.sum(warped[int(warped.shape[0]-(level+4)/4*window_height):int(warped.shape[0]-level/4*window_height),:], axis=0)
